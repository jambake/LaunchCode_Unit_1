...
public class PathFinder {
    ...
    public static void main(String[] args) {
        ...
        In in = new In(ArgsProcessor.chooseFile("datafiles/pathfinder")); 
        Graph G = GraphGenerator.read(in, "/");
        String s = ap.nextString("Starting at:");
        PathFinder pf = new PathFinder(G, s);
        String t = ap.nextString("Enter target");
        while (t != null && !t.trim().isEmpty()) {
             for (String v : pf.pathTo(t)) {
                StdOut.println("   " + v);
            }
            StdOut.println("distance " + pf.distanceTo(t));
            t = ap.nextString("Enter another target:");
        }
    }
